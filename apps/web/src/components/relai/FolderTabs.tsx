...
/**
 * FolderTabs — top-of-folder-page tabs (Search · Outreach · Profile)
 * plus the Outreach + Profile tab bodies. Ported from the design
 * bundle's folder-tabs.jsx.
 */
...
export type FolderTab = "search" | "outreach" | "profile";
...
export function FolderTabs({
  ...
  counts: { outreach?: number };
}) {
  const tabs: Array<{ id: FolderTab; label: string; sub: string; count?: number | undefined }> = [
    ...
    <div className="ft-tabs">
      {tabs.map((t) => (
        ...
        >
          ...
          {t.count != null && t.count > 0 && <span className="ft-tab-count mono">{t.count}</span>}
        </button>
      ))}
    </div>
  );
...
export function OutreachTab({
  ...
  const [filter, setFilter] = useState<"all" | "pdf" | "email">("all");
  const filtered = artifacts.filter((a) => (filter === "all" ? true : a.kind === filter));
  const firstName = folder.clientName.split(" ")[0];

  const segments: Array<["all" | "pdf" | "email", string, number]> = [
  ...
  return (
    <div className="ft-outreach">
      <div className="ft-section-hdr">
        <div>
          <div className="ft-section-title serif">Outreach for {firstName}.</div>
          <div className="ft-section-sub">
            Packets RelAI generated, emails it drafted, and what {firstName} actually opened.
          </div>
        </div>
        <div className="ft-segctl">
          {segments.map(([id, lbl, n]) => (
            <button
              key={id}
              type="button"
      ...
      <div className="ft-artifact-grid">
        ...
        <div className="ft-artifact-new">
          ...
          <div className="ft-artifact-new-l2">
            Generate a packet or draft an email from any saved listings.
          </div>
    ...
    <div className="ft-artifact">
      ...
      {isPdf ? (
        <div className="ft-artifact-preview pdf">
          ...
          <div className="ft-artifact-mini">
            <div className="ft-artifact-mini-img" />
          ...
          <div className="ft-artifact-mini">
            <div className="ft-artifact-mini-img alt" />
            <div className="ft-artifact-mini-pricerow">
              <span>$795k</span>
        ...
        <div className="ft-artifact-preview email">
          <div className="ft-email-line">
            <b>Subject:</b> {a.title}
          </div>
          <div className="ft-email-line">
            <b>To:</b> sarah.johnson@…
          </div>
          <div className="ft-email-body">
            &quot;Hi Sarah — two from this week I think you&apos;ll love.{" "}
            <span className="ft-hl">2412 Pine</span> hits everything: Fitler Square, garage, private
            patio. <span className="ft-hl">318 S 22nd</span> is older stock with a courtyard, and
            worth a look despite 27 days on market.&quot;
          </div>
        </div>
      )}
      ...
      <div className="ft-artifact-meta">
        {isPdf ? (
          ...
          <span className="ft-meta-item">
            <b>{a.sentenceCount}</b> sentences
          </span>
        )}
        {a.openedByClient ? (
          <span className="ft-meta-open">
            <span className="ft-dot ok" />
    ...
    <div className="ft-profile">
      <div className="ft-profile-hero">
        <div className="ft-profile-hero-l">
          ...
          <div className="ft-profile-notes">{folder.notes}</div>
        ...
        <div className="ft-profile-hero-r">
          <div className="ft-profile-pulse">
            <div className={`ft-pulse-dot ${folder.pulse}`} />
            <div>
      ...
      <div className="ft-profile-grid">
        <section className="ft-profile-col">
          ...
          <div className="ft-ledger-v2">
            {ledger.map((r) => (
              <div key={`${r.k}:${r.src}`} className={`ft-ledger-row ${r.dir}`}>
        ...
        <section className="ft-profile-col">
          ...
          <div className="ft-timeline">
            {profileEvents.map((e, i) => (
              <div key={`${e.ts}:${e.kind}`} className={`ft-tl-row k-${e.kind}`}>
                <div className="ft-tl-rail">
                  <span className="ft-tl-dot" />
                  {i < profileEvents.length - 1 && <span className="ft-tl-line" />}
                </div>

...
function FolderPage({ folder, data, onBack }) {
  const [rawInput, setRawInput] = React.useState("3-bed house in Fitler Square or Rittenhouse, under $900k, needs a garage and outdoor space. They love old homes with character.");
  ...
  function runSearch() {
    setThinking(true);
    setSearched(false);
    setTimeout(() => {setThinking(false);setSearched(true);}, 900);
  }
  ...
  function hideListing(id, reason) {
    const n = new Set(hidden);n.add(id);setHidden(n);setHidingId(null);
  }
  ...
  return (
    <div className="content">
      ...
      {tab === "profile" &&
      <ProfileTab folder={folder} profileEvents={data.profileEvents || []} ledger={data.preferenceLedger} />
      }
      ...
      <div className="folder-page">
        <div className="folder-main">
          ...
          {searched && <ExtractedBlock criteria={criteria} ambiguity={ambiguity} onResolveAmbig={(id) => {
            setAmbiguity({ ...ambiguity, options: ambiguity.options.map((o) => ({ ...o, picked: o.id === id })) });
          }} />}

          {thinking &&
          <div className="thinking">
              Reading {folder.clientName.split(" ")[0]}'s brief<span className="dots"></span><br />
          ...
          <>
            ...
            <div className="selbar">
                  <span className="count"><b>{selected.size}</b> listing{selected.size === 1 ? "" : "s"} selected for {folder.clientName.split(" ")[0]}</span>
                  <div className="spacer"></div>
  ...
  return (
    <div className="extracted">
      <div className="hdr">
        ...
        {criteria.ambiguities.length > 0 && <span className="badge amb mono">{criteria.ambiguities.length} UNCLEAR</span>}
        <span className="spacer"></span>
        <span className="toggle">what the AI heard</span>
      </div>
      <div className="chips">
        {criteria.hard.locations.map((l) => <span key={l} className="chip-criteria extr">{l} <span className="x">×</span></span>)}
        <span className="chip-criteria extr">≤ ${(criteria.hard.priceMax / 1000).toFixed(0)}k <span className="x">×</span></span>
        <span className="chip-criteria extr">{criteria.hard.bedsMin}+ bd <span className="x">×</span></span>
        <span className="chip-criteria extr">{criteria.hard.bathsMin}+ ba <span className="x">×</span></span>
        <span className="chip-criteria extr">Garage <span className="x">×</span></span>
      ...
      <div className="ambig-resolver">
          ...
          <button key={o.id} className={"opt " + (o.picked ? "pick" : "")} onClick={() => onResolveAmbig(o.id)}>
                {o.label}
              </button>
          )}
          </div>
...
function ListingRow({ listing, rank, selected, favorite, saved, expanded, hidingOpen, onToggleSel, onToggleFav, onToggleSave, onExpand, onOpenHide, onHide }) {
  const l = listing;
  const priceFmt = new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 }).format(l.price);
  ...
  (l.whyHighlights || []).forEach(([phrase]) => {
    ...
    if (idx >= 0) {
      ...
      whySpans.push(<span key={phrase} className="hl">{txt.slice(idx, idx + phrase.length)}</span>);
  ...
  return (
    <div className={"listing " + (selected ? "sel" : "")}>
        <div className="top">
          ...
          onClick={onToggleSel}
          style={l.photoUrls && l.photoUrls[0] ? {
        ...
        <div className="body" onClick={onToggleSel} style={{ cursor: "pointer" }}>
          <div className="priceline">
            ...
            {saved && !favorite && <span style={{ color: "var(--accent)", fontSize: 11 }}>● Saved</span>}
          </div>
          <div className="addr">{l.address} · {l.neighborhood} · {l.zip}</div>
          <div className="stats mono">
            <b>{l.beds}</b> BD · <b>{l.baths}</b> BA · <b>{l.sqft.toLocaleString()}</b> SF · BUILT <b>{l.yearBuilt}</b> · {l.dom}d ON MARKET
          </div>
          <div className="why">{whySpans}</div>
          {l.stretch && <div className="stretch">⚠ {l.stretch}</div>}
          {l.negotiable && <div className="stretch" style={{ background: "var(--moss-soft)", color: "var(--moss)" }}>↓ {l.negotiable}</div>}
        </div>
        <div className="right">
          <button className="match" onClick={onExpand} title="Click to explain">
            <div className={"num " + cls}>{l.match}</div>
            <div className="lbl">MATCH <span className="info">ⓘ</span></div>
          </button>
          <div className="actions">
            <button className={"fav " + (favorite ? "on" : "")} onClick={onToggleFav} title="Favorite">
      ...
      <div className="hide-reasons">
          ...
          <div className="opts">
            {["Wrong block / neighborhood", "Too small", "Bad light / photos", "Tenant-occupied", "Already seen it", "Wrong property type", "Other…"].map((r) =>
          <button key={r} className="opt" onClick={() => onHide(r)}>{r}</button>
          )}
          </div>
...
function ScoreExplain({ listing: l }) {
  ...
  return (
    <div className="explain">
      <div className="hdr">
        <span>HEURISTIC <b>{l.heuristic.toFixed(2)}</b></span>
        <span>·</span>
        <span>SEMANTIC <b>{l.semantic.toFixed(2)}</b></span>
        <span>·</span>
        <span>BLEND <b>w = 0.20</b></span>
      </div>
      <div className="rows">
        {rows.map((r) =>
        <div key={r.k} className="explain-row">
            <span className="lbl">{r.k}</span>
            <div className="track"><div className={"fill " + r.c} style={{ width: r.v * 100 + "%" }}></div></div>
            <span className="val">{r.v.toFixed(2)}</span>
          </div>
        )}
        <div className="explain-row">
          <span className="lbl">Pref boost</span>
          <div className="track"><div className={"fill " + (l.preferenceBoost >= 0 ? "moss" : "rose")} style={{ width: Math.abs(l.preferenceBoost) * 500 + "%" }}></div></div>
          <span className={"val " + (l.preferenceBoost >= 0 ? "up" : "dn")}>{prefSign}{Math.abs(l.preferenceBoost).toFixed(2)}</span>
        </div>
        <div className="explain-row sum">
          <span className="lbl">Final match</span>
          <div className="track"><div className="fill" style={{ width: l.match + "%" }}></div></div>
          <span className="val">{l.match}</span>
        </div>
      </div>
      <div className="explain-foot">
        SEMANTIC STATE: <span className={"tag " + l.semanticState}>{l.semanticState.toUpperCase()}</span>
        · "applied" means the model compared this listing's text to the folder's taste vector and returned a similarity score.
      </div>
    </div>);
  ...
  return (
    <div className="card sc-summary">
      <div className="sc-summary-hdr">
        ...
        <button className="sc-summary-edit" onClick={onOpen} style={{ backgroundColor: "rgb(55, 107, 182)", color: "rgb(250, 250, 250)" }}>Edit →</button>
      </div>
      <div className="sc-summary-row">
        <div className="sc-summary-col">
          ...
          <div className="sc-summary-chips">
            {top3pos.map((c) =>
            <span key={c.id} className="sc-chip mini positive">{c.label}</span>
            )}
        ...
        <div className="sc-summary-col">
          ...
          <div className="sc-summary-chips">
            ...
            {top3avo.map((c) =>
            <span key={c.id} className="sc-chip mini avoidance">{c.label}</span>
            )}
      ...
      <div className="sc-summary-foot mono">
        TASTE VECTOR · LIVE · CLICK EDIT TO RE-RANK
      </div>
    </div>);
...
function TasteCard({ folder }) {
  ...
  return (
    <div className="card">
      ...
      <div className="taste-sub">Built from {folder.savedCount} saves, {folder.favoriteCount} favorites, and 3 hides across 3 searches.</div>
      {bars.map((b) =>
      <div key={b.k} className="bar-row">
          <span className="lbl">{b.k}</span>
          <div className="tk"><span style={{ width: b.v * 100 + "%" }}></span></div>
    ...
    <div className="card">
      ...
      <div className="ledger">
        ...
        <div key={i} className="row">
            <div className="k">{r.k} <em>{r.delta}</em></div>
            <div className="v">
  ...
  return (
    <div className="mapview">
      <div className="m-left">
        {listings.map((l, i) =>
        <div key={l.id} className={"item " + (selectedId === l.id ? "active" : "")} onClick={() => onSelect(l.id)}>
            <div className={"thumb " + (l.photos[0] || "")}></div>
            <div>
              <div className="px">${(l.price / 1000).toFixed(0)}k <span className="match">MATCH {l.match}</span></div>
              <div className="addr">{l.address.split(",")[0]} · {l.neighborhood}</div>
              <div className="stats">{l.beds} BD · {l.baths} BA · {l.sqft.toLocaleString()} SF · {l.dom}d</div>
            </div>
          </div>
        )}
      </div>
      <div className="m-canvas">
        ...
        <div className="label" style={{ top: "22%", left: "8%" }}>Schuylkill R.</div>
        <div className="label" style={{ top: "12%", left: "44%" }}>Rittenhouse Sq</div>
        <div className="label" style={{ top: "56%", left: "14%" }}>Fitler Square</div>
        <div className="label" style={{ top: "76%", left: "50%" }}>Graduate Hosp.</div>
        {listings.map((l, i) =>
        <button key={l.id}
...
function PacketModal({ folder, listings, onClose }) {
  const [opts, setOpts] = React.useState({
    cover: true, photos: true, remarks: true, walkscore: false, compare: true
  });
    ...
    <div className="scrim" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        <div className="m-hdr">
          ...
          <div className="sub mono">{pageCount} PAGES · {listings.length} LISTINGS</div>
        ...
        <div className="m-body">
          <div className="preview-area">
            <div className="pages">
              ...
              <div className="preview-page cover">
                  ...
                  <div className="hdr-serif" style={{ marginTop: 4 }}>{folder.clientName}</div>
                  <div style={{ marginTop: 10 }}>
                    <div className="lines">
                      <span className="l"></span><span className="m"></span><span className="l"></span>
                      <span className="s"></span>
                    </div>
                  </div>
                  <div style={{ marginTop: 10, fontSize: 6.5, fontStyle: "italic", color: "var(--ink-3)" }}>
                    "Three Fitler Square rowhomes I think you'll love, plus two Rittenhouse trinities worth a second look."
                  </div>
                  <div className="foot"><span>RELAI</span><span>COVER</span></div>
                </div>
              }
              {listings.map((l, i) =>
              <div key={l.id} className="preview-page">
                  <div className="eyebrow">No. {String(i + 1).padStart(2, "0")} · match {l.match}</div>
                  <div className={"img " + (l.photos[0] || "")}></div>
                  <div className="big">{l.address.split(",")[0]}</div>
                  <div className="med">${(l.price / 1000).toFixed(0)}k · {l.beds}bd · {l.baths}ba · {l.sqft.toLocaleString()} SF</div>
                  {opts.remarks &&
                <div className="lines" style={{ marginTop: 5 }}>
                      <span className="l"></span><span className="m"></span><span className="s"></span>
                    </div>
                }
                  <div className="foot"><span>{l.neighborhood}</span><span>P. {i + (opts.cover ? 2 : 1)}</span></div>
                </div>
              )}
              {opts.compare &&
              <div className="preview-page">
                  <div className="eyebrow">Side-by-side</div>
                  <div className="hdr-serif" style={{ fontSize: 9, marginTop: 3 }}>Comparison</div>
                  <div className="lines" style={{ marginTop: 6 }}>
                    <span className="l"></span><span className="l"></span><span className="l"></span>
                    <span className="m"></span><span className="m"></span><span className="m"></span>
                    <span className="s"></span>
                  </div>
                  <div className="foot"><span>COMPARE</span><span>P. {pageCount}</span></div>
                </div>
              }
            </div>
          </div>
          <div className="controls">
            ...
            <div className={"opt " + (opts.walkscore ? "on" : "")} onClick={() => toggle("walkscore")}>
            ...
            <div className="meta mono">
              ~1.8 MB · READY IN 2–4s<br /><br />
              RelAI will write the cover letter by summarizing why these {listings.length} listings suit {folder.clientName.split(" ")[0]}'s taste profile.
            </div>
          </div>
        </div>
